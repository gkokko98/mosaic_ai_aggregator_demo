...
export function WideAppCard({ app, from, className = "" }: WideAppCardProps) {
  ...
  return (
    <Link
      to={`/app/${app.id}`}
      state={{ from }}
      className={`flex flex-col gap-3 rounded-2xl bg-surface p-4 ${className}`}
    >
      ...
      <span className="w-full rounded-full bg-accent py-3 text-center text-sm font-bold uppercase tracking-wide text-app-bg">
        Subscribe now
      </span>
    </Link>
  );
